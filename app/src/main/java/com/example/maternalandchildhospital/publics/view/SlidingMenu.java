...
public class SlidingMenu extends HorizontalScrollView {
	...
	private View homeBackgroundLayout;
	...
	public SlidingMenu(Context context, AttributeSet attrs, int defStyle) {
		...
		a.recycle();
		distance = Utils.dpToPx(context, 15);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (!once) {
			...
			homeBackgroundLayout = mContent.findViewById(R.id.home_background_layout);
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		scale = 77 * (1 - scale);
		int color = ColorUtils.setAlphaComponent(0x000000, (int) scale);
		homeBackgroundLayout.setBackgroundColor(color);
