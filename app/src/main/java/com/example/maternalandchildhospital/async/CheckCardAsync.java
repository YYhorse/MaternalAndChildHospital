 ...
 *         信息检查
 */
public class CheckCardAsync extends AsyncTask<Integer, Integer, String> {
	public String api_intf = "/user/checkCard";
	...
	public CheckCardAsync(boolean isShowDialog, String userId, String userSessionId, String idCard, Context context, UpdateUi uu) {
		updateUi = uu;
	...
	protected void onPostExecute(String str) {
		...
		try {
			...
			if ("0000".equals(respCode)) {
				...
				Utils.Log("CheckCardAsync da = " + da);
				if (!Utils.strNullMeans(da)) {
					JSONObject data = new JSONObject(da);
					// 1: 找到该身份证的注册信息，跳往首页 2: 该身份证已经被他人使用。 0：没找到，继续填写建卡信息
					String isMatch = data.optString("isMatch");
					if("1".equals(isMatch)){
						String yunId = data.optString("yunId");
					}
					
					updateUi.updateUI(isMatch);
				} else {
